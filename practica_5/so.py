...
class KillInterruptionHandler(AbstractInterruptionHandler):
    ...
    def execute(self, irq):
        ...
        self.kernel.memoryManager.freeBlock(pcb.baseDir, len(pcb.instructions))
        log.logger.info(self.kernel.memoryManager)
...
class Kernel():
    ...
    def __init__(self, scheduler=FirstComeFirstServedScheduler(), fitStrategy=FirstFitStrategy()):
        ...
        self._memoryManager = MemoryManager(self, fitStrategy)
        self._crontab = Crontab(self)
    ...
    def runCompactation(self):
        log.logger.info("Running compactation...")
        self.memoryManager.reset()
        lastBaseDir = 0
        pcbRunningWait = self.runningPCB
        pcbRunningWait.state = READY
        DISPATCHER.save(self.runningPCB)
        for pcb in self.pcbTable.pcbList:
            if pcb.state == READY:
                log.logger.debug("Moving PCB: {pid} from {baseDir} to {lastBaseDir}".format(pid=pcb.pid, baseDir=pcb.baseDir, lastBaseDir=lastBaseDir))
                pcb.baseDir = lastBaseDir
                baseDir = LOADER.load(pcb)
                pcb.baseDir = baseDir 
                lastBaseDir += len(pcb.instructions)
        pcbRunningWait.state = RUNNING
        DISPATCHER.load(pcbRunningWait)
        log.logger.info("Compactation finished, new baseDir: {lastBaseDir}".format(lastBaseDir=lastBaseDir))
        return lastBaseDir
