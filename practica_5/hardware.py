...
class InterruptVector():
    ...
    def handle(self, irq):
        log.logger.debug("Handling {type} irq with parameters = {parameters}".format(type=irq.type, parameters=irq.parameters ))
        self.lock.acquire()
        try:
    ...
    def tick(self, tickNbr):
        if (self._busy):
            ...
            if (self._ticksCount > self._deviceTime):
                ...
                log.logger.error("device {deviceId} - Busy: {ticksCount} of {deviceTime}".format(deviceId = self.deviceId, ticksCount = self._ticksCount, deviceTime = self._deviceTime))
...
class Hardware():
    ...
    def switchOn(self):
        log.logger.warning(" ---- SWITCH ON ---- ")
        return self.clock.start()

    def switchOff(self):
        self.clock.stop()
        log.logger.warning(" ---- SWITCH OFF ---- ")
