...
class PCB:
    ...
    def __init__(self, path, baseDir = None, priority=0, instructions=[]):
    ...
    def __repr__(self):
        return "PID: {pid}, Path: {path}, Priority: {priority}, State: {state}, page_table: {page_table}, PC: {pc}"\
            .format(pid=self.pid, pc=self.pc, state=self.state, path=self.path, page_table= self.page_table, priority=self.priority, age=self.age, instructions=self._instructions)
...
class PcbTable:
    ...
    @property
    def pcbList(self):
        return list(self._pcbTable.values())
