...
class MMU():
    ...
    def fetch(self,  logicalAddress):
        ...
        #
        # calculamos la pagina y el desplazamiento correspondiente a la direccion logica recibida 
        page_index = logicalAddress // self._frameSize
        displacement = logicalAddress % self._frameSize
        #
        # buscamos la direccion Base del frame donde esta almacenada la pagina
        try:
            frameId = self._tlb[page_index]
        except:
            raise Exception("\n*\n* ERROR \n*\n Error en el MMU\nNo se cargo la pagina  {pageId}".format(pageId = str(page_index)))
        #
        ##calculamos la direccion fisica resultante
        frameBaseDir  = self._frameSize * frameId
        physicalAddress = frameBaseDir + displacement
        #
